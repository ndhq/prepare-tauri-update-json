...
async function createTauriRelease() {
  ...
  const promises = latestAssets.map(async (asset) => {
    ...
    if (/x64\.app\.tar\.gz$/.test(name)) {
      darwinX86_64.url = browser_download_url;
    } else if (/universal\.app\.tar\.gz$/.test(name)) {
      darwinX86_64.url = browser_download_url;
    }
    if (/x64\.app\.tar\.gz\.sig$/.test(name)) {
      darwinX86_64.signature = await getAssetSign(browser_download_url);
    } else if (/universal\.app\.tar\.gz\.sig$/.test(name)) {
      darwinX86_64.signature = await getAssetSign(browser_download_url);
    }
  ...
  const releaseData = {
    version: process.env.VERSION_TAG || latestTag.name,
